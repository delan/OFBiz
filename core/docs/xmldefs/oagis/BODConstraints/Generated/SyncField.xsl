...
<axsl:stylesheet xmlns:axsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sch="http://www.ascc.net/xml/schematron" xmlns:oa="http://www.openapplications.org/oagis" version="1.0" oa:dummy-for-xmlns="">
   ...
   <axsl:template match="/">
      <schematron-output title="Schematron Validator for OAGI Constraints" schemaVersion="" phase="#ALL">
         ...
         <active-pattern name="FieldValue Level">
            <axsl:apply-templates/>
         </active-pattern>
         <axsl:apply-templates select="/" mode="M3"/>
      </schematron-output>
   </axsl:template>
   <axsl:template match="oa:Field" priority="4000" mode="M2">
      <fired-rule id="" context="oa:Field" role=""/>
      <axsl:choose>
         <axsl:when test="oa:FieldValue"/>
         <axsl:otherwise>
            <failed-assert id="" test="oa:FieldValue" role="">
               <axsl:attribute name="location">
                  <axsl:apply-templates select="." mode="schematron-get-full-path"/>
               </axsl:attribute>
               <text>Must have a FieldValue.</text>
            </failed-assert>
         </axsl:otherwise>
      </axsl:choose>
   ...
   <axsl:template match="oa:FieldValue" priority="4000" mode="M3">
      <fired-rule id="" context="oa:FieldValue" role=""/>
      <axsl:choose>
         ...
         <axsl:otherwise>
            <failed-assert id="" test="oa:NameValue" role="">
               ...
               <text>Must have an "NameValue".</text>
            </failed-assert>
         </axsl:otherwise>
      </axsl:choose>
