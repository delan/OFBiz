...
/**
 * Creates objects for a cache.  The cache is essentially a map,
 * so this factory is given a "key" and creates the appropriate "value"
 * which will be cached.  There are a number of other functions that
 * can be overridden for more specific control, but createObject is
...
public abstract class CachedObjectFactory {
     ...
     * object (so long as the original can be recovered in translateObject.
     * This will be called whenever an object is returned to the client, whether
     * a new object or a cached object.
     * @param cachedObject The object in the cache, as created by createObject.
     * @return The object to return to the client.  If different, the cached
     *    object must be recoverable by translateObject.
     */
     ...
     * and a client attempt to release an object at the same time.  In this
     * case, this method may work, return null, or throw an exception and the
     * cache will handle it gracefully.  The default implementation returns the
     ...
     * @return The cached object, as originally returned by createObject
     */
     ...
     * createObject or prepareObject (on behalf of this cache).
     * @param cache The cache that is closing.  You may decide to allow
     *    multiple caches you use your factory, or to restrict it to a one-to-one
     *    relationship.
     * @throws java.lang.IllegalArgumentException
     ...
     * is called when the cache is full and new objects are added, and when
     * the cache is closed.
     */
