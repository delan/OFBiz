...
The Initial Developer of the Original Code is ________________________. Portions created by ______________________ are Copyright © _____________________________. All Rights Reserved.

Contributor(s): ______________________________________.

Alternatively, the contents of this file may be used under the terms of the _____ license (the "[___] License"), in which case the provisions of [______] License are applicable instead of those above. If you wish to allow use of your version of this file only under the terms of the [____] License and not to allow others to use your version of this file under the DPL, indicate your decision by deleting the provisions above and replace them with the notice and other provisions required by the [___] License. If you do not delete the provisions above, a recipient may use your version of this file under either the DPL or the [___] License.'
...
@import url(</xsl:text><xsl:value-of select="$externalCSSURL"/><xsl:text>);
         ...
         <body>
            ...
            <xsl:choose>
               ...
               <xsl:otherwise>
                  ...
                  <xsl:apply-templates select="xsd:complexType | xsd:element | xsd:group | xsd:notation | xsd:simpleType" mode="topSection"/>
               </xsl:otherwise>
            </xsl:choose>
         ...
         <xsl:choose>
            ...
            <xsl:when test="normalize-space(translate($sortByComponent,'TRUE','true'))='true'">
               ...
               <xsl:if test="xsd:attributeGroup or xsd:complexType or xsd:group or xsd:notation or xsd:simpleType">
                  <li><a href="#SchemaDefinitions">Global Definitions</a>
                     <ul>
                        <!--DEJ <xsl:apply-templates select="xsd:attributeGroup | xsd:complexType | xsd:group | xsd:notation | xsd:simpleType" mode="toc"> -->
                        <xsl:apply-templates select="xsd:complexType | xsd:group | xsd:notation | xsd:simpleType" mode="toc">
                           <xsl:sort select="local-name(.)" order="ascending"/>
                           <xsl:sort select="@name" order="ascending"/>
                        </xsl:apply-templates>
                     </ul>
            ...
            <xsl:otherwise>
               <li><a href="#SchemaComponents">Global Schema Components</a>
                  <ul>
                     <!--DEJ <xsl:apply-templates select="xsd:attribute | xsd:attributeGroup | xsd:complexType | xsd:element | xsd:group | xsd:notation | xsd:simpleType" mode="toc"/> -->
                     <xsl:apply-templates select="xsd:complexType | xsd:element | xsd:group | xsd:notation | xsd:simpleType" mode="toc"/>
                  </ul>
               </li>
            </xsl:otherwise>
         </xsl:choose>
   ...
   <xsl:template match="xsd:*[@name]" mode="topSection">
      <!-- Header -->
      <!--DEJ <xsl:call-template name="ComponentSectionHeader">
         <xsl:with-param name="component" select="."/>
      </xsl:call-template>
      -->
      ...
      <!-- Footer -->
      <!--DEJ <xsl:call-template name="SectionFooter"/> -->
   </xsl:template>
   ...
   <xsl:template name="SampleInstanceTable">
      ...
      <xsl:if test="local-name($component)!='simpleType' and local-name($component)!='notation'">
         ...
         <xsl:call-template name="CollapseableBox">
            ...
            <!--DEJ <xsl:with-param name="caption">XML Instance Representation</xsl:with-param> -->
