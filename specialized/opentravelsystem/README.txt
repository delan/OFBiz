...
Welcome to the OpenTravelsystem Component				Last changed 2005/01/09
...
similar to the lines which are already there
...
Clicking on the WebSite tab, will start the Hotel E-commerce frontend application
...
you use the 'screen' widget. In the controller.xml there is a <view-map link to the *screens.xml file and this file is 
pointing to a form or for the older applications to a ftl/bsh file. 
WEB-INF/pagedef/*.xml files should not be used anymore, this functionalitry is taken over by the 'actions' 
in the screen or form widget.

However we do not live in an ideal world, so currently the ecommerce application (not the component) is
completely copied and changed to the opentravelsystem needs.
In the backend application we only have the changed screens, it uses functions from the partymgr, 
product/catalog and order components.
...
application/images the ofbiz-coponent.xml
